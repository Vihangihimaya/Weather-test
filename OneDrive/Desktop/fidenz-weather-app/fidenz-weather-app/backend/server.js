...
// Weather API route with SIMPLE MFA
app.get('/api/weather', async (req, res) => {
  try {
    // Check if MFA code is provided
    const mfaCode = req.query.mfa_code;
    const testEmail = "careers@fidenz.com";
    
    if (!mfaCode) {
      // Generate and display MFA code when no code is provided
      const otp = Math.floor(100000 + Math.random() * 900000).toString();
      
      console.log(` MFA CODE for ${testEmail}: ${otp}`);
      console.log(` In real app, this would be sent to: ${testEmail}`);
      console.log(` Use this URL: http://localhost:5000/api/weather?mfa_code=${otp}`);
      console.log(` For frontend: http://localhost:5173?mfa_code=${otp}`);
    ...
    // SIMPLE MFA VERIFICATION - Any 6-digit code works for testing
    if (mfaCode.length === 6 && !isNaN(mfaCode)) {
      // If MFA is valid, return weather data
      const weatherData = await getWeatherData();
      res.json(weatherData);
    } else {
      return res.status(401).json({ 
        error: 'INVALID_MFA',
        message: 'Invalid MFA code. Please enter a 6-digit number.'
      });
    }
...
app.listen(PORT, () => {
  ...
  console.log(` MFA Enabled: Email verification required`);
  console.log(`  Weather API: http://localhost:${PORT}/api/weather`);
  console.log(`  Original Cities Data: http://localhost:${PORT}/api/cities-original`);
});